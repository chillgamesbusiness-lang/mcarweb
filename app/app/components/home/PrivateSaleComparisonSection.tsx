      ...
      <div className="mx-auto max-w-[1280px] relative">
        ...
        <ScrollReveal delay={0.1}>
          <div className="grid md:grid-cols-2 gap-5 max-w-4xl mx-auto">
            {/* Private sale column */}
            <div className="card-premium p-0 overflow-hidden opacity-60">
              <div className="px-6 py-4 border-b border-warm-border/40 bg-surface-warm/50">
                <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-warm-gray">Private sale</span>
              </div>
              {rows.map((row, i) => (
                <div key={row.label} className={`px-6 py-4 ${i < rows.length - 1 ? 'border-b border-warm-border/20' : ''}`}>
                  <p className="text-[11px] font-bold uppercase tracking-wider text-warm-gray/50 mb-1">{row.label}</p>
                  <p className="text-[14px] text-warm-gray leading-snug">{row.them}</p>
                </div>
              ))}
            </div>

            {/* MCar column */}
            <div className="card-premium p-0 overflow-hidden ring-1 ring-gold/20 shadow-lg shadow-gold/[0.06]">
              <div className="px-6 py-4 border-b border-gold/10 bg-gold/[0.04]">
              ...
              {rows.map((row, i) => (
                <div key={row.label} className={`px-6 py-4 ${i < rows.length - 1 ? 'border-b border-warm-border/20' : ''}`}>
                  ...
                  <p className="text-[14px] text-charcoal-deep font-semibold leading-snug flex items-center gap-2">

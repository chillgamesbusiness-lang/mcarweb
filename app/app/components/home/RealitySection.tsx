      ...
      <div className="mx-auto max-w-[1280px] relative">
        <div className="grid lg:grid-cols-[1fr_1fr] gap-12 lg:gap-20 items-center">
          ...
          <ScrollReveal>
            ...
            <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-extrabold tracking-[-0.02em] text-charcoal-deep leading-[1.12] mb-8">
              Most people lose <span className="gradient-gold-text">£500–£2,000</span> selling their car.
            </h2>
            <div className="space-y-4 text-[17px] sm:text-[19px] text-warm-gray leading-relaxed">
          ...
          <ScrollReveal delay={0.15}>
            <div className="card-premium p-0 overflow-hidden">
              <div className="px-6 py-4 border-b border-warm-border/40">
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-gold-dark">What we check</span>
              </div>
              {[
                ...
                <div key={item.label} className={`px-6 py-5 flex items-start gap-4 ${i < 2 ? 'border-b border-warm-border/30' : ''}`}>
                  <span className="text-[11px] font-mono text-gold/40 mt-0.5 select-none">{String(i + 1).padStart(2, '0')}</span>
                  <div>
                    <p className="text-[14px] font-bold text-charcoal-deep mb-0.5">{item.label}</p>
                    <p className="text-[13px] text-warm-gray leading-snug">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
